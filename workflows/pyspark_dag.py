from airflow import DAG
from datetime import timedelta
from airflow.utils.dates import days_ago
from airflow.providers.google.cloud.operators.dataproc import (
    DataprocStartClusterOperator,
    DataprocStopClusterOperator,
    DataprocSubmitJobOperator,
)

# Variables
PROJECT_ID = "batchdp"
REGION = "us-central1"
CLUSTER_NAME = "my-demo-cluster"
COMPOSER_BUCKET = "us-central1-batchdp-0390f10f-bucket"  # Change if needed

# Define PySpark jobs
PYSPARK_JOB_1 = {
    "reference": {"project_id": PROJECT_ID},
    "placement": {"cluster_name": CLUSTER_NAME},
    "pyspark_job": {"main_python_file_uri": f"gs://{COMPOSER_BUCKET}/data/INGESTION/hospitalA_mysqlToLanding.py"},
}
PYSPARK_JOB_2 = {
    "reference": {"project_id": PROJECT_ID},
    "placement": {"cluster_name": CLUSTER_NAME},
    "pyspark_job": {"main_python_file_uri": f"gs://{COMPOSER_BUCKET}/data/INGESTION/hospitalB_mysqlToLanding.py"},
}
PYSPARK_JOB_3 = {
    "reference": {"project_id": PROJECT_ID},
    "placement": {"cluster_name": CLUSTER_NAME},
    "pyspark_job": {"main_python_file_uri": f"gs://{COMPOSER_BUCKET}/data/INGESTION/claims.py"},
}
PYSPARK_JOB_4 = {
    "reference": {"project_id": PROJECT_ID},
    "placement": {"cluster_name": CLUSTER_NAME},
    "pyspark_job": {"main_python_file_uri": f"gs://{COMPOSER_BUCKET}/data/INGESTION/cpt_codes.py"},
}

# Default args
ARGS = {
    "owner": "Ravi SEO",
    "start_date": days_ago(1),
    "depends_on_past": False,
    "email": ["***@gmail.com"],
    "email_on_failure": False,
    "email_on_retry": False,
    "retries": 1,
    "retry_delay": timedelta(minutes=5),
}

with DAG(
    dag_id="pyspark_dag",
    schedule_interval=None,
    description="DAG to start Dataproc cluster, run PySpark jobs, and stop it",
    default_args=ARGS,
    catchup=False,
    tags=["pyspark", "dataproc", "etl", "marvel"],
) as dag:

        start_cluster = DataprocCreateClusterOperator(
    task_id="start_cluster",
    project_id=PROJECT_ID,
    region=REGION,
    cluster_name=CLUSTER_NAME,
    cluster_config={
        "master_config": {"num_instances": 1, "machine_type_uri": "n1-standard-2"},
        "worker_config": {"num_instances": 2, "machine_type_uri": "n1-standard-2"},
    },
    use_if_exists=True,   # supported here

    )

    pyspark_task_1 = DataprocSubmitJobOperator(
        task_id="pyspark_task_1", job=PYSPARK_JOB_1, region=REGION, project_id=PROJECT_ID
    )
    pyspark_task_2 = DataprocSubmitJobOperator(
        task_id="pyspark_task_2", job=PYSPARK_JOB_2, region=REGION, project_id=PROJECT_ID
    )
    pyspark_task_3 = DataprocSubmitJobOperator(
        task_id="pyspark_task_3", job=PYSPARK_JOB_3, region=REGION, project_id=PROJECT_ID
    )
    pyspark_task_4 = DataprocSubmitJobOperator(
        task_id="pyspark_task_4", job=PYSPARK_JOB_4, region=REGION, project_id=PROJECT_ID
    )

    stop_cluster = DataprocStopClusterOperator(
        task_id="stop_cluster",
        project_id=PROJECT_ID,
        region=REGION,
        cluster_name=CLUSTER_NAME,
        trigger_rule="all_done",   # ensure cluster always stops
    )

    # DAG ordering
    start_cluster >> pyspark_task_1 >> pyspark_task_2 >> pyspark_task_3 >> pyspark_task_4 >> stop_cluster
